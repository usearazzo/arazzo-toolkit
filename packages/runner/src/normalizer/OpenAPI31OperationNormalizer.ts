...
class OpenAPI31OperationNormalizer {
  ...
  async normalize(
    ...
    try {
      ...
      throw new NormalizationError(
        `Failed to evaluate path item pointer "${pathItemPointer}" in OpenAPI 3.1 document at "${document.uri}"`,
        {
          cause: error,
    ...
    if (!isPathItemElement(pathItem)) {
      throw new NormalizationError(
        `Pointer "${pathItemPointer}" does not reference a path item in OpenAPI 3.1 document at "${document.uri}"`,
        {
          operationId: toValue(operation.operationId),
          uri: document.uri,
          operationPointer,
    ...
    const entry = document.parseResult.api as OpenApi3_1Element;
  ...
  async #dereferencePathItemParameters(
    ...
    try {
      ...
      throw new NormalizationError(
        `Failed to dereference Path Item parameters in OpenAPI 3.1 document at "${document.uri}"`,
