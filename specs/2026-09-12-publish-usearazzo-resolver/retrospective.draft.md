...
- `plan.md` tasks 4 and 5: `resolveArazzoElement` / `resolveOpenAPIElement` cannot forward a seeded `dereference.refSet` to `resolveApiDOM`; the upstream resolve strategies deep-merge a fresh `ReferenceSet` over it (ramda `mergeDeepRight`), which strips the prototype. The element variants pass only `resolve.baseURI` and `parse.mediaType`.
- `requirements.md` § Out of Scope "No runner changes" and `plan.md` task 11: `packages/runner/package.json` had its exact `@usearazzo/resolver` pin bumped to `1.0.1-alpha.2`; without it `npm install` cannot match the workspace package and falls back to the registry (404). The runner's parser pin stays for Phase 5.
- `plan.md` task 13: the lockfile refresh also removed a nested registry copy of `@usearazzo/parser@1.0.1-alpha.0` under `packages/resolver/node_modules`, which the stale pin had been pulling in place of the workspace package.
